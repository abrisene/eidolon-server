...
schema.methods.getCustomer = async function(email, source) {
  ...
  try {
    ...
    if (this.customerIds.stripe) {
      ...
      if (!customer) throw 'Could not retrieve customer.';
    } else {
      customer = await client.customers.create({ email, source });
      if (!customer) throw 'Could not create customer.';
...
schema.methods.createCharge = async function({
  ...
  try {
    ...
    if (!customer) throw 'Could not create charge: Could not retrieve customer.';
...
schema.statics.hashPassword = async function(password) {
  try {
    return bcrypt.hash(password, constUser.password.hashCost);
  } catch(err) {
    console.error(err);
  }
};
...
schema.statics.setPasswordWithToken = async function(tokenHash, password) {
  ...
  try {
    ...
    if (!token.type === constUser.token.resetPassword) throw `Token ${token} (${token.type}) cannot be used to reset password.`;
