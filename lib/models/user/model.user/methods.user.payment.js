...
schema.methods.addPaymentSource = async function (provider = 'stripe', source) {
...
schema.methods.createOrder = async function (provider = 'stripe', items, source) {
  const Order = this.db.model('Order');
  let customerId = this.customerIds[provider];
  try {
    if (!items) throw new Error('Cannot create new order without items defined.');
    getProvider(provider);
    ...
    const order = new Order({
      ...
      email: this.email,
      userId: this._id,
      customerId: source ? undefined : customerId,
    });

    if (Array.isArray(items)) {
      order.items = items;
    } else if (typeof items === 'string') {
      order.items = [{ parent: items }];
    } else {
      throw new Error('Cannot create order with unknown item shape.');
    }
    ...
    const payment = await order.pay(source);
...
/* schema.methods.createOrderStripe = async function (source) {
  try {
    const { client } = getProvider('stripe');
    const Order = this.db.model('Order');
