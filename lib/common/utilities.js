 ...
 * @param  {boolean} indexArrays  Will convert arrays to integer indexed objects if true. 
 * @return {object}               Returns a object with no undefined keys.
 */
const trimObject = (object, indexArrays = true) => {
  const result = Object.keys(object).reduce((a, key) => {
    const obj = { ...a };
    const value = object[key];

    if (Array.isArray(value) && value.length > 0) {
      obj[key] = !indexArrays ? value : value.reduce((a, b, i) => ({ ...a, [i]: b }), {});
...
/**
This is the previous version, which
